...
def cross_entropy(L_policy, l_policy, L_idx):
    p = L_policy[:L_idx]
...
def score_frame_fmetric(model, history, ix, r, d, interp_func, mode='actor'):
    # r: radius of blur
    # d: density of scores (if d==1, then get a score for every pixel...
    #    if d==2 then every other, which is 25% of total pixels for a 2D image)
    assert mode in ['actor', 'critic'], 'mode must be either "actor" or "critic"'
    ...
    L_policy = softmax(L.data[0].numpy())
    ...
    for i in range(0,80,d):
        for j in range(0,80,d):
            ...
            if (dP > 0):
                K = cross_entropy(L_policy, l_policy, L_idx)
